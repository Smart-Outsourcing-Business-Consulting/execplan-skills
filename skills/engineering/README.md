...
The `execplan-handoff` skill is only for temporary conversation continuity
under `/tmp/codex-handoffs/`; it does not create or refresh ExecPlan lifecycle
handoff files. Lifecycle handoff preparation is governed by `PLANS.md`.
...
- **[execplan-handoff](./execplan-handoff/SKILL.md)** — Temporary conversation handoff for context-limit, pause, or later-continuation cases, with ExecPlan-aware artifact references and `/tmp/codex-handoffs/` storage; not for `implementation-prompt.md` refresh.
...
- **[execplan-zoom-out](./execplan-zoom-out/SKILL.md)** — Conditional current-state mapping for stale prompts, dirty worktrees, unclear affected files, or suspected drift; do not invoke it merely because an implementation prompt is being prepared.
